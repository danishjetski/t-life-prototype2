...
export default function SchedulePage({ weeklyData = weeklyTimetable, onEventClick, userKey = 'guest', timetableSynced = true }) {
    ...
    const [dbEvents, setDbEvents] = useState(() => getAllEvents());
    ...
    // Listen for database event updates
    useEffect(() => {
        ...
        const interval = setInterval(checkForDbUpdates, 500);
        window.addEventListener('taylor-events-updated', checkForDbUpdates);

        return () => {
            clearInterval(interval);
            window.removeEventListener('taylor-events-updated', checkForDbUpdates);
        };
    }, []);
    ...
    const slotEventMap = useMemo(() => {
        ...
        if (weekConfig.week === 8) {
            freeSlots.forEach((slot) => {
                map[slot.id] = dbEvents.filter((evt) => evt.date === dayDateISO && isEventWithinSlot(evt.time, slot.time));
            });
            return map;
        }

        const pool = dbEvents.filter((evt) => evt.category === 'focus' || evt.category === 'balance');
        freeSlots.forEach((slot, slotIndex) => {
            ...
            for (let i = 0; i < count; i += 1) {
                ...
                if (!baseEvent) continue; // Skip if pool is empty
        ...
        return map;
    }, [dayDateISO, freeSlots, selectedDay, weekConfig.week, dbEvents]);

    const totalMatchingEvents = Object.values(slotEventMap).reduce((sum, list) => sum + list.length, 0);

    return (
        <div className="px-5 pt-6 pb-24 bg-[#050508] text-white h-auto min-h-screen w-full">
            {/* Header */}
            <div className="flex items-center justify-between mb-5">
                <div>
                    <h1 className="text-base font-outfit font-bold text-white">Schedule</h1>
                    <p className="text-xs font-inter text-gray-500">Week {weekConfig.week} • {formatWeekRange(weekConfig.startDate)}</p>
                </div>
