...
export default function EventFeed({ mode, onCheckIn, onEventClick, userKey = 'guest' }) {
    ...
    const filteredEvents = useMemo(() => dbEvents.filter((e) => e.category === category), [dbEvents, category]);
    ...
    // Listen for changes to the local events database
    useEffect(() => {
        ...
        // Check for updates every 500ms (lightweight polling from localStorage)
        const interval = setInterval(checkForDbUpdates, 500);
        
        // Also listen for custom events if we emit them
