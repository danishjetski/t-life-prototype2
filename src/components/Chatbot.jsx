                    ...
                    >
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#0a0506]">
                            {messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[85%] rounded-2xl px-4 py-2 text-xs ${msg.role === 'user' ? 'bg-taylor-red text-white rounded-br-none' : 'bg-white/10 text-gray-200 rounded-bl-none'}`}>
                                        {msg.content}
                                    </div>
                                </div>
                            ))}
                            {loading && (
                                <div className="flex justify-start">
                                    <div className="bg-white/10 text-gray-200 rounded-2xl rounded-bl-none px-4 py-2 text-xs animate-pulse">
                                        Thinking...
                                    </div>
                                </div>
                            )}
                        ...
                        <div className="p-3 bg-[#1a1a24] border-t border-white/5 flex gap-2">
                            <input
                                ...
                                className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-taylor-red"
                            />
                            <button
                                onClick={handleSend}
