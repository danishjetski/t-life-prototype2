...
import { login as dbLogin, createStudent, getStudents, getAdmins } from '../data/db';
...
const LoginPage = ({ onLogin }) => {
    ...
    const handleForgotPassword = (e) => {
        ...
        const accountExists = students.some(s => s.email.toLowerCase() === trimmedEmail) || admins.some(a => a.email.toLowerCase() === trimmedEmail);
        ...
        setSuccessMsg('Reset instructions have been sent to your email if the account exists on file.');
    };

    return (
        <div className="min-h-screen bg-[#050508] text-white flex flex-col items-center py-12 px-6 relative overflow-y-scroll font-sans pb-24" style={{ WebkitOverflowScrolling: 'touch', overscrollBehavior: 'contain' }}>
            ...
            <div className="z-10 max-w-md w-full flex flex-col items-center">
                ...
                >
                    <span className="text-2xl font-serif font-bold text-white">T</span>
                </motion.div>
                ...
                {(authView === 'signin' || authView === 'signup') && (
                    ...
                    >
                        <button 
                            ...
                            className={`flex-1 py-1.5 text-[10px] font-inter font-bold rounded-lg transition-colors ${authView === 'signin' ? 'bg-taylor-red text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                        >
                            Sign In
                        </button>
                        <button 
                            onClick={() => { setAuthView('signup'); setError(''); setSuccessMsg(''); }}
                            className={`flex-1 py-1.5 text-[10px] font-inter font-bold rounded-lg transition-colors ${authView === 'signup' ? 'bg-taylor-red text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                        >
                            Sign Up
                        </button>
                    </motion.div>
                ...
                <AnimatePresence mode="wait">
                    {authView === 'signin' ? (
                        ...
                        >
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-xs focus:outline-none focus:border-taylor-red/50 text-white transition-colors"
                                        value={email}
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-inter text-gray-400 ml-1">Password</label>
                                <div className="relative">
                                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" size={18} />
                                    <input 
                                        type={showPassword ? 'text' : 'password'} 
                                        placeholder="••••••••" 
                                        className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-10 pr-12 text-xs focus:outline-none focus:border-taylor-red/50 text-white transition-colors"
                                        value={password}
                            ...
                            <div className="flex justify-between items-center text-[10px] text-gray-400">
                                ...
                                <button 
                                    type="submit"
                                    className="w-full py-2.5 mt-4 bg-taylor-red hover:bg-taylor-red-light text-white rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors shadow-glow-red"
                                >
                                ...
                                <div className="my-4 flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] text-gray-500 font-inter">OR</span>
                                    <div className="flex-1 h-px bg-white/10"></div>
                                </div>
                        ...
                        >
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-xs focus:outline-none focus:border-taylor-red/50 text-white"
                                        value={regName}
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        className="w-full bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-xs focus:outline-none focus:border-taylor-red/50 text-white appearance-none"
                                        style={{ color: '#ffffff', backgroundColor: 'rgba(255,255,255,0.06)' }}
                                        value={regProgramme}
                                        onChange={(e) => setRegProgramme(e.target.value)}
                                        required
                                    >
                                        <option value="" disabled style={{ color: '#9ca3af', backgroundColor: 'rgba(15, 23, 42, 0.95)' }}>Select your programme</option>
                                        {programmes.map((programme) => (
                                            <option key={programme} value={programme} style={{ color: '#ffffff', backgroundColor: 'rgba(15, 23, 42, 0.95)' }}>{programme}</option>
                                        ))}
                                    </select>
                                </div>
                            </div>
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        placeholder="••••••••" 
                                        className="w-full bg-white/5 border border-white/10 rounded-xl py-2 pl-10 pr-12 text-xs focus:outline-none focus:border-taylor-red/50 text-white"
                                        value={regPassword}
                        ...
                        >
                            ...
                            <button
                                type="submit"
                                    className="w-full py-2.5 mt-4 bg-taylor-red hover:bg-taylor-red-light text-white rounded-xl font-bold text-xs transition-colors shadow-glow-red"
                            >
                                Send Reset Link
                            </button>
                            <button
                                type="button"
                                onClick={() => { setAuthView('signin'); setError(''); setSuccessMsg(''); }}
                                    className="w-full py-2.5 bg-white/5 hover:bg-white/10 text-gray-200 rounded-xl font-bold text-xs transition-colors"
                            >
                                Back to Sign In
                            </button>
                        </motion.form>
                ...
                >
                    <div className="flex items-center gap-3 mb-4">
                        <div className="flex-1 h-px bg-white/10"></div>
                        <span className="text-[10px] uppercase tracking-wider font-inter text-gray-500">Admin Test Credentials</span>
                        <div className="flex-1 h-px bg-white/10"></div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        >
                            <div className="flex items-center gap-3 text-left">
                                ...
                                <div>
                                    <p className="text-xs font-outfit font-bold text-white">Event Manager</p>
                        ...
                        >
                            <div className="flex items-center gap-3 text-left">
                                ...
                                <div>
                                    <p className="text-xs font-outfit font-bold text-white">Super Admin</p>
